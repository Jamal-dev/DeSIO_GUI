function compare_tt_deformation()
% =========================================================================
...
var_name = 'tower_top_displacement';
arrnode  = [2+21];
...
%             run(['m' model.strSimName '_model'])
        ...
        res_de  = fun_extract_wt_tip_deformation(q,time,arrnode,3); 
...
if flag_plot_ED == 1
    ...
    for i = 1:size(strInpVarED{1},1)
        if strncmp('YawBrTDxp',strInpVarED{1}(i),9) == 1; arrcolumnED(1,1) = i; end
        if strncmp('YawBrTDyp',strInpVarED{1}(i),9) == 1; arrcolumnED(1,2) = i; end
        if strncmp('YawBrTDzp',strInpVarED{1}(i),9) == 1; arrcolumnED(1,3) = i; end
    end
    cd(currDir);
...
if flag_plot_BD == 1
    ...
    for i = 1:size(strInpVarBD{1},1)
        if strncmp('YawBrTDxp',strInpVarBD{1}(i),9) == 1; arrcolumnBD(1,1) = i; end
        if strncmp('YawBrTDyp',strInpVarBD{1}(i),9) == 1; arrcolumnBD(1,2) = i; end
        if strncmp('YawBrTDzp',strInpVarBD{1}(i),9) == 1; arrcolumnBD(1,3) = i; end
    end
    cd(currDir);
...
%% Plotting time series
        ...
        subplot(3,1,1); hold on; grid on; title(['Tower-top u_1']); ylabel('u_1 in m'); xlabel('time in s');
        subplot(3,1,2); hold on; grid on; title(['Tower-top u_2']); ylabel('u_2 in m'); xlabel('time in s');
        subplot(3,1,3); hold on; grid on; title(['Tower-top u_3']); ylabel('u_3 in m'); xlabel('time in s');

        % DeSiO
        if flag_plot_De == 1
            strWhatToComp = [strWhatToComp '_DeSiO' ];

            subplot(3,1,1); plot(time,res_de(i).u(:,1),'-r','linewidth',lineSize);
            handlePlotDe1 = plot(time(inztime2plotDe),res_de(i).u(inztime2plotDe,1),'.','color','k','marker','o','markersize',markerSize);

            subplot(3,1,2); plot(time,res_de(i).u(:,2),'-g','linewidth',lineSize);
        ...
        if flag_plot_ED == 1
            ...
            subplot(3,1,2); plot(ofastED_res(:,1),ofastED_res(:,arrcolumnED(i,2)),'-g','linewidth',lineSize);
            ...
            subplot(3,1,3); plot(ofastED_res(:,1),ofastED_res(:,arrcolumnED(i,3)),'-b','linewidth',lineSize);
        ...
        if flag_plot_BD == 1
            ...
            subplot(3,1,1); plot(ofastBD_res(:,1),ofastBD_res(:,arrcolumnBD(i,1)),'-r','linewidth',lineSize);
            ...
            subplot(3,1,2); plot(ofastBD_res(:,1),ofastBD_res(:,arrcolumnBD(i,2)),'-r','linewidth',lineSize);
            ...
            subplot(3,1,3); plot(ofastBD_res(:,1),ofastBD_res(:,arrcolumnBD(i,3)),'-r','linewidth',lineSize);
...
%% plotting mean value
        ...
        figure(); 
        subplot(3,1,1); hold on; grid on; title(['Tower-top u_1']); ylabel('mean u_1 in m'); 
        subplot(3,1,2); hold on; grid on; title(['Tower-top u_2']); ylabel('mean u_2 in m');
        subplot(3,1,3); hold on; grid on; title(['Tower-top u_3']); ylabel('mean u_3 in m');
