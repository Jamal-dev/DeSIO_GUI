% =================================================================================================================
function DeSiO_Aero_plot_G_time(currDir,strfilename,rings,time,rgbColor,plotStyle)
cd = currDir;
...
circs = fun_load_file([model.strSimName '_uvlm_circs_cp.dres']);
...
leg = {};
for i = 1:length(rings)
    plot(t,field(:,rings(i)),'linewidth',2,'color',rgbColor(i,:),'linestyle',plotStyle{i});
...
print([strfilename '_G_vs_time'],'-dpng', '-r500');
close all;

fid = fopen([strfilename '_G.txt'],'w');
for i = 1:length(inz)
    for j = 1:length(rings)
        if j~=length(rings)
